...
try:
    print("...Loading wrapper from torch-radon v1...")
    from wrappers.basic_wrapper import BasicWrapper  # provide on-the-fly radon/iradon transform
except ImportError:
    print("...Loading wrapper from torch-radon v2...")
    from wrappers.basic_wrapper_v2 import BasicWrapper
...
class CrossFourierConv2d(nn.Module):
    ...
    def forward(self, x: torch.Tensor, y: torch.Tensor):
        ...
        #ffted = ffted.view((eff_batch_size, -1,) + ffted.size()[3:])  # (batch, 2c, h, w/2+1)
        ...
        #ffted = ffted.view((eff_batch_size, -1, 2,) + ffted.size()[2:]).permute(0, 1, 3, 4, 2).contiguous()  # (batch,c, t, h, w/2+1, 2)
...
class WindowAttention(nn.Module):
    ...
    def forward(self, qkv:torch.Tensor, use_fast_attn=False):
        ...
        if AVAIL_FAST_ATTN and use_fast_attn:
            # [TODO]: F.sdpa leads to significant performance drop in multi-setting training, looking into why
...
class FeatureFuser(nn.Module):
    def __init__(self, in_dims, out_dim=None, fusion_type='cat'):
        super().__init__()
        if fusion_type == 'add':
...
class Encoder(nn.Module):
    def __init__(
        ...
        **block_kwargs):
        ...
        self.enc_layer1 = BasicLayer(**common_kwargs, **layer_kwargs[0], **block_kwargs)
        self.enc_down1 = PatchEmbed(patch_size=2, in_channels=embed_dims[0], embed_dim=embed_dims[1])  # downsampling
        
        self.enc_layer2 = BasicLayer(**common_kwargs, **layer_kwargs[1], **block_kwargs)
        self.enc_down2 = PatchEmbed(patch_size=2, in_channels=embed_dims[1], embed_dim=embed_dims[2])  # downsampling
        
        # bottleneck
        self.neck = BasicLayer(**common_kwargs, **layer_kwargs[2], **block_kwargs)
...
class Decoder(nn.Module):
    def __init__(
        ...
        **block_kwargs):
        ...
        fusion_dict = dict(fusion_type=fusion_type)
        common_kwargs = dict(
            network_depth=network_depth, attn_loc='last', window_size=window_size, cond_dim=cond_dim, 
            use_bounded_scale=use_bounded_scale, use_scale_for_mlp=use_scale_for_mlp, use_rope=use_rope)
        layer_kwargs = [
        ...
        self.dec_layer1 = BasicLayer(**common_kwargs, **layer_kwargs[3], **block_kwargs)
        ...
        self.dec_layer2 = BasicLayer(**common_kwargs, **layer_kwargs[4], **block_kwargs)
...
class ProCTV2(BasicWrapper):
    def __init__(
        ...
        **wrapper_kwargs):
        ...
        decoder_dict = dict(fusion_type=fusion_type)
        ...
        if use_learnable_prompt:
            # two task: sparse-view and limited-angle, this can be modified
            self.prompt = nn.Parameter(torch.zeros(1, 2, 5, 720).float(), requires_grad=True)
        else:
            self.prompt = None
