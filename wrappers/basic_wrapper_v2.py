...
from torch_radon import Volume2D
from torch_radon import FanBeam as RadonFanbeam


class BasicWrapper(nn.Module):
    """
    Provide functions for on-the-fly radon/iradon operations with TorchRadon V2.
    Source: https://github.com/carterbox/torch-radon
    """
    def __init__(
        ...
        volume = Volume2D()
        volume.set_size(height=img_size, width=img_size)
        self.radon_tool = RadonFanbeam(self.det_count, self.full_angles, self.source_distance, volume=volume)
    ...
    def fbp(self, sinogram: torch.Tensor, angles=None):
        '''sinogram to ct image'''
        angles = self.full_angles if angles is None else angles
        angles = torch.FloatTensor(angles).to(sinogram.device) if not torch.is_tensor(angles) else angles
        filtered_sinogram = self.radon_tool.filter_sinogram(sinogram, "ram-lak")
        back_proj = self.radon_tool.backward(filtered_sinogram, angles=angles)
        return back_proj
    
    def fp(self, ct_image: torch.Tensor, angles=None):
        '''ct image to sinogram'''
        angles = self.full_angles if angles is None else angles
        angles = torch.FloatTensor(angles).to(ct_image.device) if not torch.is_tensor(angles) else angles
        sinogram = self.radon_tool.forward(ct_image, angles=angles)
        return sinogram
    ...
    @staticmethod
    def add_gaussian_noise_to_sinogram(sinogram: torch.Tensor, sigma=0.01):
        if sigma > 0:
            smin, smax = sinogram.min(), sinogram.max()
