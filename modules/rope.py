""" 
Source: https://github.com/huggingface/pytorch-image-models/blob/2ec2f1aa73e3976553b1ddcb4245a42052a59138/timm/layers/pos_embed_sincos.py
...
def build_fourier_pos_embed(
    ...
    if bands is None:
        ...
        device = bands.device if device is None else device
        dtype = bands.dtype if dtype is None else dtype
...
def apply_rot_embed(x: torch.Tensor, sin_emb: torch.Tensor, cos_emb: torch.Tensor):
    if sin_emb.ndim == 3:
...
def apply_rot_embed_cat(x: torch.Tensor, emb: torch.Tensor):
...
def apply_keep_indices_nlc(x, pos_embed: torch.Tensor, keep_indices: torch.Tensor):
