...
from utilities.rec_loss import get_metrics, VGGPerceptualLoss, MSSSIMLoss, SSIMLoss
...
class ProCTTrainer(BasicTrainer):
    # to include limited-angle task
    ...
    def prepare_dataset(self):
        ...
        self.train_dataset = SimpleCTDataset(
            root_dir=dataset_root_dir, img_list_info=opt.train_json, img_shape=self.img_size,
            mode='train', num_train=opt.num_train, min_hu=opt.min_hu, max_hu=opt.max_hu, clip_hu=opt.clip_hu)
        self.val_dataset = SimpleCTDataset(
            root_dir=dataset_root_dir, img_list_info=opt.val_json, img_shape=self.img_size,
            mode='val', num_val=opt.num_val, min_hu=opt.min_hu, max_hu=opt.max_hu, clip_hu=opt.clip_hu)
    ...
    def get_input_target_pair(self, mus, task_param=None, task='sparse_view', prob_flip=0):
        supp_size = self.opt.support_size  # number of in-context pairs
        
        if mus.device.type == 'cpu':
            mus = mus.to(self.device, non_blocking=True)
        
        if (prob_flip > 0) and (np.random.rand() <= prob_flip):
            mus = torch.flip(mus, dims=[-1])
            
        if self.opt.use_phantom:
            # In this case, `mus` only contains the source images, and we need to concatenate the 
            # phantom as contextual images.
            batch_size = mus.shape[0]
            batch_size_supp = batch_size * supp_size
            eff_batch_size = batch_size + batch_size_supp  # B*(S+1)
            phantom = self.phantom.to(self.device, non_blocking=True).repeat(batch_size_supp, 1, 1, 1)
            mus = torch.cat([mus, phantom], dim=0)
        else:
            # In this case, `mus` already contains the source and contextual images.
            eff_batch_size = mus.shape[0]  # B*(S+1)
            batch_size = eff_batch_size // (supp_size + 1)
            batch_size_supp = eff_batch_size - batch_size
        
        # `mus` should have the shape of [B*(S+1), 1, H, W], containing both source and contextual images,
        # so they can be pre-processed in a parallel way
        assert mus.shape[0] == eff_batch_size, f'Effective batch size mismatch: expected {eff_batch_size}, got {mus.shape}.'
        
        if self.multigpu:
            inputs, targets = self.net.module.generate_input_target_mu(mus, task_param, task=task, return_sinogram=False)
        else:
            inputs, targets = self.net.generate_input_target_mu(mus, task_param, task=task, return_sinogram=False)
        
        input_data, target_data = inputs[:batch_size], targets[:batch_size]  # [B, 1, H, W]
        input_data_supp, target_data_supp = inputs[-batch_size_supp:], targets[-batch_size_supp:]
        #target_data_sino = sinograms[:batch_size]
        if self.unsqueeze:
            input_data_supp = E.rearrange(input_data_supp, "(b s) c h w -> b s c h w", b=batch_size)
            target_data_supp = E.rearrange(target_data_supp, "(b s) c h w -> b s c h w", b=batch_size)
            
        support_data = torch.cat([input_data_supp, target_data_supp], dim=-3) # [B, S, 2*C, H, W]
        
        sinogram_mask = self.get_sino_mask(task_param, task=task)
        prompt = self.mask2prompt(sinogram_mask)
        prompt = prompt.float().repeat_interleave(batch_size, dim=0).to(input_data.device)
        
        return input_data.detach(), target_data.detach(), prompt.detach(), support_data.detach()
    ...
    def get_fused_loss(self, preds, labels, reduction='mean'):
    ...
    def fit(self,):
        ...
        _batch_size = opt.batch_size if opt.use_phantom else opt.batch_size * (opt.support_size + 1)
        ...
        if self.multigpu:
            ...
            self.train_loader = DataLoader(
                self.train_dataset, batch_size=_batch_size, num_workers=opt.num_workers, sampler=self.train_sampler, pin_memory=True, drop_last=True)
            self.val_loader = DataLoader(self.val_dataset, batch_size=_batch_size, num_workers=opt.num_workers, sampler=self.val_sampler,)
        else:
            self.train_loader = DataLoader(self.train_dataset, batch_size=_batch_size, num_workers=opt.num_workers, pin_memory=True, shuffle=True, drop_last=True)
            self.val_loader = DataLoader(self.val_dataset, batch_size=_batch_size, num_workers=opt.num_workers)
    ...
    def train(self,):
        ...
        pbar = tqdm.tqdm(self.train_loader, leave=False) if opt.use_tqdm else self.train_loader
        for i, images_ in enumerate(pbar): 
            for task, task_param_list in self.task_kwargs.items():
                if self.epoch > opt.warmup_steps:  # when it is stable, train with all settings
                    if task == 'sparse_view':
                        ...
                        if task_param % 2 and task_param > 144:  # deal with torch-radon bug, probability some division issues
                            task_param -= 1
                    else:  # warmup by training with a few settings
                ...
                images, labels, prompt, context = self.get_input_target_pair(images_, task_param, task=task, prob_flip=opt.prob_flip)
                ...
                loss = self.get_fused_loss(preds, labels, reduction='mean') * w
    ...
    def val(self,):
        ...
        for task, task_param_list in task_kwargs.items():
            for task_index, task_param in enumerate(task_param_list):
                ...
                for _, images_ in enumerate(pbar):  # one batch contain different images
            
                    images, labels, prompt, context = self.get_input_target_pair(images_, task_param, task=task)           
